...
const ReportAccountButton = ({ username, publicName }: ReportAccountButtonProps) => {
	...
	const isOwner = Boolean(viewerUsername && viewerUsername === profileUsername);
	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		setSubmitting(true);
		try {
			const response = await fetch(`${saasBase}/api/creators/report`, {
				method: "POST",
				credentials: "include",
				...
				<DialogContent className="max-w-md">
					<form onSubmit={handleSubmit} className="gap-4 flex flex-col">
						...
						<DialogFooter>
							...
							<Button type="submit" disabled={submitting || !reason || isPending}>
