...
const SuccessContent = () => {
	...
	const addedDollars = amountParam && /^\d+$/.test(amountParam) ? Number(amountParam) : null;
	...
	const subtitle =
		addedDollars != null && addedDollars > 0
			? `Your +$${addedDollars.toLocaleString("en-US")} bid increase is live. Your ranking will update as the board recalculates.`
			: "Your bid increase is live. Your ranking will update as the board recalculates.";
	...
	return (
		<div className="relative overflow-hidden">
			...
			<div className="max-w-170 p-12 shadow-hover bg-b-surface4 before:left-6 before:right-6 before:h-3.75 before:bg-b-surface2 max-md:px-8 max-md:pb-8 max-md:before:hidden relative mx-auto rounded-4xl before:absolute before:top-full before:-z-1 before:rounded-b-4xl">
				...
				<p className="text-body-lg text-t-secondary leading-relaxed text-center">{subtitle}</p>
				<div className="mt-10 gap-3 flex flex-wrap justify-center">
					<Button isSecondary as="link" href="/dashboard">
						Return to dashboard
					</Button>
				</div>
			</div>
...
const BidIncreaseSuccessPage = () => {
	return (
		<Layout isLoggedIn isFixedHeader isHiddenFooter>
...
export default BidIncreaseSuccessPage;
